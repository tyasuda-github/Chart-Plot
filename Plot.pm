...
#   v 0.12 - 2024/12/20 tyasuda
#            - タイトルを指定していると、data2pxl が返す y 座標がずれるバグを修正。
#            - プロットのサイズを $plot_size で一括指定し、4 を 8 に変更。
#            - データがない場合に原点にプロットしてしまうバグを修正。
#            - プロットの緑色が明るすぎる（薄い）ので 255 を 127 に変更。

package Chart::Plot;

$Chart::Plot::VERSION = '0.12'; 
...
sub _increase_vert_offset_and_recalculate_conversion_constants{
	my $self = shift;
	$self->_getMinMax() unless $self->{'_validMinMax'};
	...
	$self->{'_valid_increase_vert_offset_and_recalculate_conversion_constants'} = 1;
}
...
sub getGDobject {
  ...
  $self->_increase_vert_offset_and_recalculate_conversion_constants()
    unless $self->{'_valid_increase_vert_offset_and_recalculate_conversion_constants'};
...
sub _init {
  ...
  $self->{'_green'} = $self->{'_im'}->colorAllocate(0,127,0);
...
sub _drawData {
  ...
  foreach $dataSetLabel (keys %{$self->{'_data'}}) {
	...
	next unless defined $self->{'_data'}->{$dataSetLabel}[0];
    ...
    $self->{'_im'}->arc($px, $py,$plot_size,$plot_size,0,360,$self->{$color})
